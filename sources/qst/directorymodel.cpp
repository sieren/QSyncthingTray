...
namespace model
{
  SyncthingDirectoryModel::SyncthingDirectoryModel(data::SyncConnection &connection, QObject *parent)
  : QAbstractItemModel(parent)
  , mDirectories(connection.directories())
  {
...
//    connect(&m_connection, &SyncthingConnection::dirStatusChanged, this, &SyncthingDirectoryModel::dirStatusChanged);
   ...
   */
  const data::SyncthingDirectory *SyncthingDirectoryModel::dirInfo(const QModelIndex &index) const
  {
    return (index.parent().isValid() ? dirInfo(index.parent())
            : (static_cast<size_t>(index.row()) < mDirectories.size() ? &mDirectories[static_cast<size_t>(index.row())] : nullptr));
  }
  ...
  QVariant SyncthingDirectoryModel::data(const QModelIndex &index, int role) const
  {
    if (index.isValid()) {
      if (index.parent().isValid()) {
        // dir attributes
        if (static_cast<size_t>(index.parent().row()) < mDirectories.size()) {
          switch (role) {
            case Qt::DisplayRole:
            case Qt::EditRole:
              if (index.column() == 0) {
       ...
       //       FALLTHROUGH;
            case DirectoryDetail:
              if (index.column() == 1 || role == DirectoryDetail) {
                // attribute values
                const auto &dir = mDirectories[static_cast<size_t>(index.parent().row())];
                switch (index.row()) {
                  case 0:
                    return dir.id;
                  case 1:
            ...
            case Qt::ToolTipRole:
              switch (index.column()) {
                case 1:
                  const SyncthingDir &dir = mDirectories[static_cast<size_t>(index.parent().row())];
                  switch (index.row()) {
                    case 3:
                      if (dir.deviceNames.isEmpty()) {
      ...
      } else if (static_cast<size_t>(index.row()) < mDirectories.size()) {
        // dir IDs and status
        const SyncthingDir &dir = mDirectories[static_cast<size_t>(index.row())];
        switch (role) {
          case Qt::DisplayRole:
          case Qt::EditRole:
            switch (index.column()) {
